...
# SLURM 资源申请配置
# ============================================================
#SBATCH --job-name=SD_LoRA_Train2     # 任务名称
#SBATCH --partition=gpu-h100          # 分区名称（使用sinfo可获取HPC支持的分区名称）
#SBATCH --qos=qos-gpu
#SBATCH --nodes=1                     # 申请 1 个节点
#SBATCH --ntasks-per-node=1           # 每个节点运行 1 个任务
#SBATCH --cpus-per-task=4             # 每个任务分配的 CPU 核心数
#SBATCH --gres=gpu:1                  # 申请 1 块 GPU（如果显存小，可以申请 H100）
#SBATCH --mem=16G                     # 申请 16GB 内存
#SBATCH --time=24:00:00               # 最大运行时间 12 小时
#SBATCH --output=logs/train2_%j.out    # 标准输出日志 (%j 会替换为任务 ID)
#SBATCH --error=logs/train2_%j.err     # 错误输出日志
