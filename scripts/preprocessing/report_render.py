...
def render_markdown(data: Dict[str, Any], fig_dir: str, md_out_dir: str) -> Tuple[str, List[str]]:
    ensure_dir(fig_dir)
    ...
    c_cat_uri = Path(c_cat_png).resolve().as_uri()
    c_reg_uri = Path(c_reg_png).resolve().as_uri()
    c_top_uri = Path(c_top_png).resolve().as_uri()
    lines: List[str] = []
...
def main() -> None:
    ...
    parser.add_argument("--fig-dir", default="report_figs", type=str)
    ...
    with open(args.out, "w", encoding="utf-8") as f:
        md, _ = render_markdown(data, args.fig_dir, out_dir)
        f.write(md)
